...
import com.espy.onecallsys.databinding.FragmentPendingOrdersBinding
import com.espy.onecallsys.ui.base.BaseFragmentWithBinding
import com.espy.onecallsys.ui.shops.adapter.PendingOrderListAdapter
...
//            (activity as OrderHistoryActivity).setToolBarProperties(args.deliveryShop?.shop_name?:getString(R.string.order_history), false)
        ...
        shopsViewModel.getPendingOrders("3", args.deliveryShop?.shopid?:"0")
    }
    ...
    private fun setObservers(){
        shopsViewModel.pendingOrderList.observe(viewLifecycleOwner, {
            (activity as OrderHistoryActivity).hideProgress()
            if (it is Result.Success && it.data.any()){
                setAutoCompleteSearch(it.data)
                loadOrders(it.data)
            }else{
                showToast("No Pending Orders found!")
            }
        })
    }

    private fun loadOrders(list: List<PendingOrder>){
        pendingOrderListAdapter = PendingOrderListAdapter(requireContext(), list){ pendingOrder ->
